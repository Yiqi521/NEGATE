...
| `conservative_negatives/definition/objective_criteria.py` | 客观判据：间隙接受、PET、RSS、起步延迟（替代/校验人工标注） |
...
| `eval/stratified_pdms.py --runs ... --flags ... [--baseline ...]` | 分层 PDMS、bootstrap CI、配对差异与拒绝准则 | run_pdm_score 的 CSV |
...
- [x] Step 3 流水线：G1–G4 + 运动学 + C1–C4（C3：行驶中 ×1.15 / 静止起步提前 1 s，参考只查 NC+TTC）；v5 正式标签 `results/negatives_navtest_v5.parquet`（3,689 条，覆盖 997/1,093 场景；G1/G2/G3b/G4 + C1–C4，C3 含到达间隙代理）
...
- [x] navtrain 训练标签 `results/negatives_navtrain_v1.parquet`（30,251 条 / 8,487 场景）+ metric cache `metric_cache_navtrain_interact`
