...
RUN apt-get update && apt-get install -y --no-install-recommends git wget ca-certificates build-essential rsync tmux \
    && rm -rf /var/lib/apt/lists/*
RUN wget -q https://repo.anaconda.com/miniconda/Miniconda3-latest-Linux-x86_64.sh -O /tmp/mc.sh \
    && bash /tmp/mc.sh -b -p /opt/conda && rm /tmp/mc.sh && conda clean -afy
...
RUN conda create -y -n navsim python=3.9 pip && conda clean -afy
...
RUN pip install --no-cache-dir -r /workspace/requirements-cu128.txt \
    ...
    && python -c "import torch; assert torch.__version__.startswith('2.7.1'), torch.__version__; assert 'sm_120' in torch.cuda.get_arch_list()"
